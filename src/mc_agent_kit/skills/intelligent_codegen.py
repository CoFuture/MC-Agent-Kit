...
提供基于 GPT/模板的代码生成、代码质量评估、代码风格检查功能。
"""
...
''',
    ...
    def generate(
        ...
        # 检查缓存
        cache_key = self._get_cache_key(prompt, strategy, style)
        ...
        elif strategy == GenerationStrategy.LLM:
            result = self._generate_from_llm(prompt, style, context)
        else:  # HYBRID
            result = self._generate_hybrid(prompt, style, context)
    ...
    def _find_matching_template(self, prompt: str) -> CodeTemplate | None:
        """查找匹配的模板"""
...
# 全局实例
_generator: IntelligentCodeGenerator | None = None
...
def get_code_generator() -> IntelligentCodeGenerator:
    ...
    if _generator is None:
        _generator = IntelligentCodeGenerator()
    return _generator
...
def generate_code(
    ...
    Returns:
        GeneratedCode: 生成的代码
    """
    return get_code_generator().generate(prompt, strategy, style)